...
public class CustomEconomy implements Economy {
    ...
    public double getBalance(OfflinePlayer player) {
        try {
            ResultSet data = db.findOne("id" ,player.getUniqueId().toString().replaceAll("-", ""));
    ...
    public EconomyResponse withdrawPlayer(OfflinePlayer player, double amount) {
        try {
            db.subtractOne(player.getUniqueId().toString().replaceAll("-", ""), amount);
    ...
    public EconomyResponse depositPlayer(OfflinePlayer player, double amount) {
        try {
            db.addOne(player.getUniqueId().toString().replaceAll("-", ""), amount);
            return new EconomyResponse(amount, 0, EconomyResponse.ResponseType.SUCCESS, null);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
